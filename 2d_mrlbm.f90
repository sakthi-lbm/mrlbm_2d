  ...
  logical, parameter :: verbose = .true.
  	...
  	if (channel_with_cylinder) then
  		nfront = L_front*ncy
   		...
   		call get_coord()
   	elseif (channel_with_square) then
   		nfront = L_front*ncy
   		nback = L_back*ncy
   		ntop = L_top*ncy
   		nbot = L_bot*ncy
...
!			ut(l) = 0.0d0
!			un(l) = 0.0d0
!			radius = sqrt( ((x(i,j) - x_c)**2) + ((y(i,j) - y_c)**2))
!			sinth = (y(i,j) - y_c)/radius 
!			costh = (x(i,j) - x_c)/radius
!			ux(i,j) = ut(l)*costh - un(l)*sinth
!			uy(i,j) = ut(l)*sinth + un(l)*costh
!			call boundary_cases_rotation_no_slip(bou_i(l),bou_j(l),label_bc(l))
		end do
	end if
	
	if(channel_with_square) then
		do l = 1,nbct
			i = bou_i(l)
			j = bou_j(l)
			ux(i,j) = 0.0d0
...
contains
	...
	do j = 1,ny
		...
		!boundary points: only boundary nodes
		isbound(cent_x - (ncy/2):cent_x + (ncy/2),cent_y - (ncy/2)) = 1		!square bottom wall
		isbound(cent_x - (ncy/2):cent_x + (ncy/2),cent_y + (ncy/2)) = 1		!square top wall
		isbound(cent_x - (ncy/2),cent_y - (ncy/2):cent_y + (ncy/2)) = 1		!square left wall
		isbound(cent_x + (ncy/2),cent_y - (ncy/2):cent_y + (ncy/2)) = 1		!square right wall
				
		!Solid points - entire square block including boundary		
		issolid(cent_x - (ncy/2):cent_x + (ncy/2), cent_y - (ncy/2):cent_y + (ncy/2)) = 1
	...
	open(unit=10, file='bound_square.dat', status="replace")
	do  i = 1, nx
		do  j = 1, ny
			if(isbound(i,j)==1) then
	...
	open(unit=20, file='solid_square.dat', status="replace")
	do  i = 1, nx
		do  j = 1, ny
			if(issolid(i,j)==1) then
	...
	open(unit=30, file='fluid_square.dat', status="replace")
	do  i = 1, nx
		do  j = 1, ny
  ...
  end subroutine get_coord_square_block
  ...
  subroutine get_coord()
	...
	open(unit=10, file='surface.dat', status="replace")
		do k = 0, nbct-1
			thet = k * (2.0d0 * PI / nbct) 
			x_s = x_c + r_c*Cos(thet) 
			y_s = y_c + r_c*Sin(thet)
			write(10,*) x_s,y_s
		end do
	close(10)
...
subroutine boundary_cases_rotation(xi,yj,label,uxp,uyp)
	implicit none
	integer,intent(in) :: xi,yj,label
	real(8),intent(in) :: uxp,uyp
	integer,dimension(0:q-1) :: Is,Os
	...
	real(8) :: num,den,om1
	real(8) :: radi,sth,cth,s2th,c2th,s4th,c4th,s6th,c6th
	
		radi = sqrt( ((x(xi,yj) - x_c)**2) + ((y(xi,yj) - y_c)**2))
		sth = (y(xi,yj) - y_c)/radi 
		cth = (x(xi,yj) - x_c)/radi
		s2th = 2.0d0*sth*cth
		c2th = cth*cth - sth*sth
		s4th = 4.0d0*(cth**3)*sth -  4.0d0*(sth**3)*cth
		c4th = (cth**4) - 6.0d0*(cth**2)*(sth**2) + (sth**4)
		s6th = 6.0d0*(cth**5)*sth -20.0d0*(cth**3)*(sth**3) + 6.0d0*(sth**5)*cth
		c6th = (cth**6) - 15.0d0*(cth**4)*(sth**2) + 15.0d0*(cth**2)*(sth**4)- (sth**6)
	...
	select case (label)
		...
		case (1) 
			...
			do k = 0, q-1
				if(Is(k)==1) then	
					rhoI_b = rhoI_b + f(xi,yj,k)
					mxyI_b = mxyI_b + f(xi,yj,k)*((Hyy(k) - Hxx(k))*0.50d0*s2th + Hxy(k)*c2th )
				end if
			end do
			mxyI_b = mxyI_b/rhoI_b
			
			om1 = (omega - 1.0d0)
			num = -36.0d0*rhoI_b*(-3.0d0 + 2.0d0*om1*c2th*mxyI_b + c4th  )
			term1 = 74.0d0 + omega + (omega - 26.0d0)*c4th
			term2 = 3.0d0*(cth + sth)*(-3.0d0*(omega - 6.0d0) + (omega - 6.0d0)*c4th &
						& + 4.0d0*(omega - 1.0d0)*s2th)
			term3 = 3.0d0*omega*(20.0d0*c4th - 3.0d0*(20.0d0 - 7.0d0*s2th + s6th))/4.0d0
			den = term1 + term2*uxp + term3*(uxp**2) 
				
			rho_prime_b = num/den
			Mxy_prime_b = (-72.0d0*rhoI_b*mxyI_b - 2.0d0*c2th*rho_prime_b  &
							& + 6.0d0*rho_prime_b*((cth - sth)**3)*uyp) &
							& / (9.0d0*rho_prime_b*(-3.0d0 + c4th ))
			
			do k = 0, q-1
				f(xi,yj,k) = w(k)*rho_prime_b*( 1.0d0 + (3.0d0*uyp*(-e(k, 1)*sth + e(k, 2)*cth)) &
								& + (4.50d0*Mxy_prime_b*( (Hyy(k) - Hxx(k))*s2th + Hxy(k)*2.0d0*c2th ) ))						
			end do
		!--------------------------------------------------------------------------------------------------------
		case (2)
		rhoI_b = 0.0d0
		...
		rhoI_b = 0.0d0
			...
			om1 = (omega - 1.0d0)
			num = 18.0d0*rhoI_b*(1.0d0 - om1*s2th*mxyI_b + 2.0d0*(cth**2)*(sth**2)  )
			term1 = 15.0d0 + (s2th**2)*om1 + 30.0d0*(cth**2)*(sth**2)
			term2 = cth*om1*s2th + 3.0d0*sth + 6.0d0*(cth**2)*(sth**3)
			term3 = 9.0d0*((cth**2) - 1.0d0)*omega*(1.0d0 + 2.0d0*(cth**2)*(sth**2))
			den = term1 + 3.0d0*term2*uxp + term3*uxp*uxp 
				
			rho_prime_b = num/den
			Mxy_prime_b = (18.0d0*rhoI_b*mxyI_b + s2th*rho_prime_b &
							& + 3.0d0*rho_prime_b*cth*uxp) &
							& / (rho_prime_b*(9.0d0 + 18.0d0*(sth**2)*(cth**2)))
			
			do k = 0, q-1
				f(xi,yj,k) = w(k)*rho_prime_b*( 1.0d0 + (3.0d0*uxp*(e(k, 1)*cth + e(k, 2)*sth)) &
								& + (3.0d0*uyp*(-e(k, 1)*sth + e(k, 2)*cth)) &
								& + (4.50d0*Mxy_prime_b*( (Hyy(k) - Hxx(k))*s2th &
								& + Hxy(k)*2.0d0*c2th ) ))						
			end do
		!--------------------------------------------------------------------------------------------------------
		...
		case (4)
			...
			om1 = (omega - 1.0d0)
			num = 72.0d0*rhoI_b*(1.0d0 + om1*c2th*mxyI_b + 4.0d0*(cth**2)*(sth**2)  )
			term1 = 2.0d0*(c2th**2)*om1 + 50.0d0*(1.0d0+ 4.0d0*(cth**2)*(sth**2))
			term2 = -5.0d0*sth + (sth**3)*(c2th - c2th*omega)
			term3 = (cth**3)*(c2th*om1 - 20.0d0*(sth**2))
			term4 = cth*(-5.0d0 + 3.0d0*c2th*om1*(sth**2))
			term5 = (cth**2)*(-3.0d0*c2th*om1*(sth) - 20.0d0*(sth**3))
			term6 = 3.0d0*omega*(-10.0d0 + 3.0d0*s2th)*(1.0d0+4.0d0*(cth**2)*(sth**2))
			den = term1 + 6.0d0*(term2 + term3 + term4 + term5)*uxp + term6*uxp*uxp 
				
			rho_prime_b = num/den
			Mxy_prime_b = (36.0d0*rhoI_b*mxyI_b - c2th*rho_prime_b  &
							& - 3.0d0*rho_prime_b*((cth - sth)**3)*uxp) &
							& / (9.0d0*rho_prime_b*(1.0d0 + 4.0d0*(sth**2)*(cth**2)))
		...
		rhoI_b = 0.0d0
			...
			om1 = (omega - 1.0d0)
			num = 18.0d0*rhoI_b*(1.0d0 + om1*s2th*mxyI_b + 2.0d0*(cth**2)*(sth**2)  )
			term1 = 15.0d0 + (s2th**2)*om1 + 30.0d0*(sth**2)*(cth**2)
			term2 = 3.0d0*cth + sth*om1*s2th + 6.0d0*(cth**3)*(sth**2)
			term3 = 9.0d0*((sth**2) - 1.0d0)*omega*(1.0d0 + 2.0d0*(cth**2)*(sth**2))
			den = term1 - 3.0d0*term2*uxp + term3*uxp*uxp 
				
			rho_prime_b = num/den
			Mxy_prime_b = (18.0d0*rhoI_b*mxyI_b - s2th*rho_prime_b &
							& + 3.0d0*rho_prime_b*sth*uxp) &
							& / (rho_prime_b*(9.0d0 + 18.0d0*(sth**2)*(cth**2)))
		...
		rhoI_b = 0.0d0
			...
			om1 = (omega - 1.0d0)
			num = 72.0d0*rhoI_b*(3.0d0 + om1*c2th*mxyI_b + 4.0d0*(cth**2)*(sth**2)  )
			term1 = (2.0d0*c2th**2)*om1 + 70.0d0*(3.0d0 + 4.0d0*(sth**2)*(cth**2))
			term2 = 3.0d0 + 4.0d0*(sth**2)*(cth**2) + c2th*om1*((cth**2) - (sth**2))
			term3 = 3.0d0*omega*(3.0d0*s2th - 2.0d0)*(3.0d0 + 4.0d0*(cth**2)*(sth**2))
			den = term1 - 6.0d0*(cth - sth)*term2*uxp + term3*uxp*uxp 
				
			rho_prime_b = num/den
			Mxy_prime_b = (36.0d0*rhoI_b*mxyI_b - c2th*rho_prime_b &
							& + 3.0d0*rho_prime_b*(((cth - sth)**2)*(cth + sth) )*uxp) &
							& / (9.0d0*rho_prime_b*(3.0d0 + 4.0d0*(sth**2)*(cth**2)))
		...
		rhoI_b = 0.0d0
			...
			num = 72.0d0*rhoI_b*(1.0d0 - om1*c2th*mxyI_b + 4.0d0*(cth**2)*(sth**2)  )
			term1 = 2.0d0*(c2th**2)*om1 + 50.0d0*(1.0d0+ 4.0d0*(cth**2)*(sth**2))
			term2 = (cth**3)*(c2th*om1 - 20.0d0*(sth**2))
			term3 = (cth**2)*sth*(3.0d0*c2th*om1 + 20.0d0*(sth**2))
			term4 = sth*(5.0d0 + c2th*om1*(sth**2))
			term5 = cth*(-5.0d0 + 3.0d0*c2th*om1*(sth**2))
			term6 = -3.0d0*omega*(10.0d0 + 3.0d0*s2th)*(1.0d0+4.0d0*(cth**2)*(sth**2))
			den = term1 - 6.0d0*(term2 + term3 + term4 + term5)*uxp + term6*uxp*uxp 
  ...
  end subroutine boundary_cases_rotation
  
  subroutine boundary_cases_rotation_no_slip(xi,yj,label)
	implicit none
	integer,intent(in) :: xi,yj,label
	integer,dimension(0:q-1) :: Is,Os
	real(8) :: rhoI_b,mxxI_b,myyI_b,mxyI_b
	...
	real(8) :: term1,term2,term3,term4,term5,term6
	real(8) :: num,den,om1
	real(8) :: radi,sth,cth,s2th,c2th,s4th,c4th
	...
	select case (label)
		...
		case (1) 
			...
			om1 = (omega - 1.0d0) 
			rho_prime_b = (-36.0d0*rhoI_b*(-3.0d0 + 2.0d0*om1*mxyI_b*c2th + c4th)) &
							& /(74.0d0 + omega + (omega - 26.0d0)*c4th)
			Mxy_prime_b = -2.0d0*(36.0d0*rhoI_b*mxyI_b + rho_prime_b*c2th)&
							& /(9.0d0*rho_prime_b*(-3.0d0 + c4th))
			
			do k = 0, q-1
				f(xi,yj,k) = w(k)*rho_prime_b*( 1.0d0 + (9.0d0*Mxy_prime_b*( (Hyy(k) - Hxx(k))*0.50d0*s2th &
								& + Hxy(k)*c2th ) ))						
			end do
		!--------------------------------------------------------------------------------------------------------
		case (2)
		...
		rhoI_b = 0.0d0
			...
			om1 = (omega - 1.0d0)
			rho_prime_b = (18.0d0*rhoI_b*(-5.0d0 + 4.0d0*om1*mxyI_b*s2th + c4th)) &
							& /(-73.0d0 - 2.0d0*omega + (2.0d0*omega + 13.0d0)*c4th)
			Mxy_prime_b = -8.0d0*(9.0d0*rhoI_b*mxyI_b + rho_prime_b*cth*sth)&
							& /(9.0d0*rho_prime_b*(-5.0d0 + c4th))
		...
		rhoI_b = 0.0d0
			...
			om1 = (omega - 1.0d0)
			rho_prime_b = (-36.0d0*rhoI_b*(-7.0d0 - 2.0d0*om1*mxyI_b*c2th + c4th)) &
							& /(244.0d0 + omega + (omega - 36.0d0)*c4th)
			Mxy_prime_b = 2.0d0*(-36.0d0*rhoI_b*mxyI_b + rho_prime_b*c2th)&
							& /(9.0d0*rho_prime_b*(-7.0d0 + c4th))
  ...
  end subroutine boundary_cases_rotation_no_slip
  
  subroutine boundary_cases_rotation_no_slip_rhoeq(xi,yj,label)
	implicit none
	integer,intent(in) :: xi,yj,label
	integer,dimension(0:q-1) :: Is,Os
	real(8) :: rhoI_b,mxxI_b,myyI_b,mxyI_b
	...
	select case (label)
		...
		case (1) 
			...
			rho_prime_b = (18.0d0*rhoI_b*(-3.0d0 - 2.0d0*mxyI_b*c2th + c4th)) &
							& /(-37.0d0 + 13.0d0*c4th)
			Mxy_prime_b = -2.0d0*(36.0d0*rhoI_b*mxyI_b + rho_prime_b*c2th)&
							& /(9.0d0*rho_prime_b*(-3.0d0 + c4th))
		...
		rhoI_b = 0.0d0
			...
			rho_prime_b = (18.0d0*rhoI_b*(-5.0d0 - 4.0d0*mxyI_b*s2th + c4th)) &
							& /(-73.0d0 + 13.0d0*c4th)
			Mxy_prime_b = -8.0d0*(9.0d0*rhoI_b*mxyI_b + rho_prime_b*cth*sth)&
							& /(9.0d0*rho_prime_b*(-5.0d0 + c4th))
		...
		rhoI_b = 0.0d0
			...
			rho_prime_b = (9.0d0*rhoI_b*(-7.0d0 + 2.0d0*mxyI_b*c2th + c4th)) &
							& /(-61.0d0 + 9.0d0*c4th)
			Mxy_prime_b = 2.0d0*(-36.0d0*rhoI_b*mxyI_b + rho_prime_b*c2th)&
							& /(9.0d0*rho_prime_b*(-7.0d0 + c4th))
  ...
  end subroutine boundary_cases_rotation_no_slip_rhoeq
...
200 format(a, T5)
  ...
  subroutine allocate_square_memory()
  	implicit none
  	
  	!Allocating variables after the get_coord subroutine execution:
  	allocate(bou_i(1:nbct),bou_j(1:nbct))
	allocate(label_bc(1:nbct))
	allocate(p_mean_cy(1:nbct))
  
  end subroutine allocate_square_memory
